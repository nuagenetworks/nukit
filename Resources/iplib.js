...
// Notation types
//IPLib.IP_UNKNOWN = 0;
IPLib.IP_DOT  = 1; // 192.168.0.42
//IP_HEX  = 2; // 0xC0A8002A
IPLib.IP_BIN  = 3; // 11000000101010000000000000101010
//IP_OCT  = 4; // 030052000052
IPLib.IP_DEC  = 5; // 3232235562
IPLib.IP_BITS = 6; // 26
...
IPLib.IPv4NetMask = function(nm, notation) {
  /*
  ...
  this.set = function(nm, notation) {
      ...
      } else if (notation == IPLib.IP_BITS) {
...
IPLib.CIDR = function(cidr) {
  this.set = function(cidr) {
      ...
      if (s.length != 2)
          throw "invalid CIDR: " + cidr;
      this.ip = new IPLib.IPv4Address(s[0]);
      this.nm = new IPLib.IPv4NetMask(s[1]);
      var ipl = this.ip.ipDec;
      var nml = this.nm.nmDec;
      this.netIpDec = (ipl & nml) >>> 0;
      this.netIp = new IPLib.IPv4Address(this.netIpDec, IPLib.IP_DEC);
      ...
      if (this.ipNum == -1 || this.ipNum == 0) {
          ...
          this.firstIp = new IPLib.IPv4Address(this.firstIpDec, IPLib.IP_DEC);
          // last ip
          if (this.ipNum == 1) {
              ...
              this.lastIp = new IPLib.IPv4Address(this.lastIpDec, IPLib.IP_DEC);
          }
      } else {
          // broadcast address
          this.bcIpDec = this.netIpDec + this.ipNum + 1;
          this.bcIp = new IPLib.IPv4Address(this.bcIpDec, IPLib.IP_DEC);
          // first ip
  ...
  this.isValidIp = function(ip) {
       ...
       */
      if (!(ip instanceof IPLib.IPv4Address) && !(ip instanceof IPLib.CIDR)) {
          if (ip.indexOf('/') == -1) {
              ip = new IPLib.IPv4Address(ip);
          } else {
              ip = new IPLib.CIDR(ip);
          }
      }
      if (ip instanceof IPLib.IPv4Address) {
          return ip.ipDec >= this.firstIpDec && ip.ipDec <= this.lastIpDec;
      } else if (ip instanceof IPLib.CIDR) {
